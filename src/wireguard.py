...
def generate_public_key(private_key: str) -> str:
    ...
    """
    return subprocess.check_output(["wg", "pubkey"], input=private_key, encoding="ascii").strip()  # noqa: S607 # nosec
...
def generate_keypair() -> WireguardKeypair:
    ...
    """
    private_key = subprocess.check_output(["wg", "genkey"], encoding="ascii").strip()  # noqa: S607 # nosec
...
def _wg_showconf(name: str) -> wgdb.WireguardLink:
    ...
    """
    conf_str = subprocess.check_output(["wg", "showconf", name], encoding="ascii")  # noqa: S607 # nosec
    config = configparser.ConfigParser()
...
def wireguard_list() -> list[wgdb.WireguardLink]:
    ...
    interfaces = [
        i.strip()
        for i in subprocess.check_output(["wg", "show", "interfaces"], encoding="ascii").split()  # noqa: S607 # nosec
    ]
...
def wireguard_syncconf(interface: wgdb.WireguardLink, is_provider: bool) -> None:
    ...
    subprocess.check_output(
        ["wg", "syncconf", interface.interface_name, "/dev/stdin"],  # noqa: S607 # nosec
